...
def filter_scores_and_topk(scores, score_thr, topk,  results=None, want_label=[]):
    ...
    # 寫在這!!!!! 用 want_labels = labels == 5(cls的編號)，然後用keep_idxs = keep_idxs[want_labels] 刪掉不要的cls的idxs，只留想要的cls的idxs。
    if want_label != []:
        want_labels = []
        for i in range(len(labels)):
            want_labels.append(labels[i] in want_label)
        for i in range(len(want_labels)):
            if want_labels[i] == False:
                keep_idxs[i] = -60
